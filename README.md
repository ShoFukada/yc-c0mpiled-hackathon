# AuthentiFlow - AI スニーカー真贋鑑定アシスタント

スニーカーの写真をアップロードすると、AI（Gemini Vision）が真贋鑑定のチェックポイントを自動特定し、インタラクティブなビジュアルガイドを生成。各ポイントのディテール撮影から真贋判定までガイドするWebアプリ。
...
4. ディテール画像のAI真贋判定（verdict / confidence / reasoning）
5. セッション履歴の一覧表示・再閲覧

## フロー

アップロード → シューズ特定 → AI解析（チェックポイント抽出） → 鑑定ガイド → ディテール撮影 → AI真贋判定
...
- SQLite + SQLAlchemy + Alembic（セッション・分析結果の永続化）
- pydantic-settings（設定管理）
- 構造化ロギング（コンソール + ファイル出力、7日ローテーション）
...
| `POST` | `/api/sessions/{id}/identify` | シューズ特定（モック） |
| `GET` | `/api/references/{filename}` | リファレンス画像の取得 |
| `POST` | `/api/sessions/{id}/inspect` | AI鑑定を実行 |
| `GET` | `/api/sessions/{id}/details` | ディテールアップロード状況・分析結果一覧 |
| `POST` | `/api/sessions/{id}/details/{point_id}` | ディテール画像のアップロード |
| `GET` | `/api/sessions/{id}/details/{point_id}/image` | ディテール画像の取得 |
| `POST` | `/api/sessions/{id}/details/{point_id}/analyze` | ディテール画像のAI真贋判定 |
...
### `POST /api/sessions/{id}/details/{point_id}/analyze` レスポンス
...
```json
{
  ...
  "verdict": "AUTHENTIC",
  "observation": "観察結果の詳細",
  "comparison": "SOPとの比較結果",
  "confidence": 85,
  "reasoning": "判定理由",
  "sop_reference": "参照したSOPセクション"
}
```
...
uv run python analyze_detail.py
uv run python guided_capture.py
```
...
│       ├── types.ts             # 共通型定義 (BBox, InspectionPoint, PointAnalysis)
│       ├── api/
│       │   ├── client.ts        # APIクライアント関数
│       │   └── queries.ts       # TanStack Query options
│       ├── routes/
│       │   ├── __root.tsx       # ルートレイアウト
│       │   ├── index.tsx        # アップロード + セッション履歴 (/)
│       │   ├── inspect.$sessionId.tsx  # プレビュー + 鑑定ガイド
│       │   └── capture.$sessionId.tsx  # ディテール撮影ウィザード
│       └── components/
│           ├── UploadScreen.tsx
│           ├── PreviewScreen.tsx
│           ├── IdentifyingScreen.tsx
│           ├── LoadingScreen.tsx
│           ├── InspectionGuide.tsx
│           └── GuidedCapture.tsx
│
...
│   │       ├── database.py      # SQLAlchemy エンジン・セッション初期化
│   │       ├── models.py        # ORM モデル定義
│   │       └── db.py            # データアクセス関数
│   ├── ai_core/
│   │   └── src/yc_hackathon_ai_core/
│   │       ├── inspector.py     # Gemini API 呼び出し・画像解析
│   │       ├── analyzer.py      # ディテール画像のAI真贋判定
